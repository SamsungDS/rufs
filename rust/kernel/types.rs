...
// SAFETY: A `None` is represented by the null pointer, which is well aligned for any
// `FOREIGN_ALIGN`. A `Some(_)` uses the non-null pointer produced by `T::into_foreign`, which
// satisfies the alignment requirement by the `ForeignOwnable` implementation of `T`.
    ...
    T: ForeignOwnable,
{
    ...
    unsafe fn from_foreign(ptr: *mut c_void) -> Self {
        if ptr.is_null() {
            ...
            // SAFETY: `ptr` is non-null, so by the safety requirements of this function, it
            // came from a call to `T::into_foreign`.
