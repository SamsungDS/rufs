...
///
/// Owned by a [`GenDisk`](super::gen_disk::GenDisk).
///
/// # Invariants
///
    ...
    T: Operations,
{
    ...
    pub fn new(
        ...
        // SAFETY: `tagset.into_raw()` produces a valid tag set pointer whose refcount we
        // transfer to the queue, `limits.as_raw()` is a valid queue limits pointer, and `data`
        // came from a call to `ForeignOwnable::into_foreign`.
        let mq = from_err_ptr(unsafe {
            bindings::blk_mq_alloc_queue(tagset.into_raw().cast_mut().cast(), limits.as_raw(), data)
        })?;
        ...
        Ok(BoundRequestQueue(
            // SAFETY: `mq` is a valid request queue allocated above, and we own the refcount
            // granted by `blk_mq_alloc_queue`.
    ...
    pub fn limits(&self) -> &Limits {
        // SAFETY: By type invariant, `self.0` is a valid `request_queue`, and `Limits` is a
        // transparent wrapper around `bindings::queue_limits`.
        unsafe { &*(&raw const (*self.0.get()).limits).cast::<Limits>() }
    }
}

// SAFETY: The C request queue embeds a reference count in `refs` that keeps the queue alive,
// and the queue is freed when the last reference is dropped in `dec_ref`.
unsafe impl<T: Operations> RefCounted for RequestQueue<T> {
    ...
    unsafe fn dec_ref(obj: NonNull<Self>) {
        // SAFETY: By the safety requirements of this function, `obj` is a valid
        // `RequestQueue`.
        let queue_ptr = unsafe { (*obj.as_ptr()).0.get() };
        // SAFETY: `queue_ptr` is a valid `request_queue`, and `Refcount` is a transparent
        // wrapper around `refcount_t`.
        let refcount_ptr = unsafe { (&raw mut (*queue_ptr).refs).cast::<Refcount>() };
        // SAFETY: `refcount_ptr` points to a live refcount embedded in the queue.
        let refcount_ref = unsafe { &*refcount_ptr };

        if refcount_ref.dec_and_test() {
            // SAFETY: `queue_ptr` is a valid `request_queue` that we hold the last reference
            // to.
...
impl<T: Operations> BoundRequestQueue<T> {
    ...
    /// Reconstruct a [`BoundRequestQueue`] from a raw request queue pointer.
    ///
    /// # Safety
    ///
    /// `request_queue` must be a pointer previously returned by
    /// [`BoundRequestQueue::into_raw`], and ownership of the binding is transferred to the
    /// returned value.
    pub(crate) unsafe fn from_raw(request_queue: *mut bindings::request_queue) -> Self {
        BoundRequestQueue(
            // SAFETY: By the safety requirements of this function, `request_queue` is valid
            // and we own the refcount held by the binding.
...
// SAFETY: The block layer synchronizes access to the underlying request queue, so the binding
// can be transferred between threads when the queue private data is `Send`.
...
// SAFETY: The block layer synchronizes access to the underlying request queue, so the binding
// can be shared between threads when the queue private data is `Sync`.
