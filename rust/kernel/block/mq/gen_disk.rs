...
impl<T: Operations> GenDisk<T> {
    ...
    pub fn new_for_queue(
        ...
        // SAFETY: `gendisk` is valid and initialized. We have exclusive access, since the disk
        // is not added to the VFS yet.
        unsafe { (*gendisk).private_data = data };

        // SAFETY: `data` was created by a call to `ForeignOwnable::into_foreign` above, and the
        // guard is dismissed before ownership of `data` is transferred to the disk.
    ...
    extern "C" fn release(this: *mut bindings::gendisk) {
        ...
        // SAFETY: By C API contract, `this` is a valid gendisk for the duration of this
        // callback.
    ...
    /// Reconstruct a reference to a [`GenDisk`] from a raw gendisk pointer.
    ///
    /// # Safety
    ///
    /// `ptr` must point to a valid `struct gendisk` created by [`GenDisk::new_for_queue`], and
    /// the gendisk must remain valid for the duration of lifetime `'a`.
...
// SAFETY: The C gendisk embeds a reference count in `part0->bd_device` that keeps the gendisk
// alive, and `put_disk` frees the gendisk when the last reference is dropped.
unsafe impl<T: Operations> RefCounted for GenDisk<T> {
    fn inc_ref(&self) {
        // SAFETY: By type invariant, `self.gendisk` is a valid gendisk, so `part0` points to a
        // valid block device whose embedded device can be referenced.
        unsafe { bindings::get_device(&raw mut (*(*self.gendisk.get()).part0).bd_device) };
    }

    unsafe fn dec_ref(obj: NonNull<Self>) {
        // SAFETY: By the safety requirements of this function, `obj` is a valid `GenDisk` with a
        // refcount that we own, so we can drop a reference with `put_disk`.
